...
<template>
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
    ...
    <Link href="/admin-pandawa/articles/create" class="inline-flex items-center justify-center px-6 py-3 border border-transparent text-sm font-medium rounded-xl text-white bg-emerald-600 hover:bg-emerald-700 shadow-md hover:shadow-lg transition-all gap-2">
  ...
  <div class="space-y-4">
    <div v-for="(article, i) in articles.data" :key="article.id" class="bg-white rounded-3xl p-6 shadow-sm border border-emerald-50 flex items-center justify-between group hover:shadow-md transition-all">
      ...
      <div class="flex items-center gap-3">
        <Link :href="`/admin-pandawa/articles/${article.id}/edit`" class="w-12 h-12 rounded-full bg-white border border-gray-200 text-gray-500 hover:text-emerald-600 hover:border-emerald-200 flex items-center justify-center transition-colors shadow-sm">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
        </Link>
        <button @click="router.delete(`/admin-pandawa/articles/${article.id}`)" class="w-12 h-12 rounded-full bg-red-50 border border-red-100 text-red-500 hover:bg-red-100 flex items-center justify-center transition-colors shadow-sm">
