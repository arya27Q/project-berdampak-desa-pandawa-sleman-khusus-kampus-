...
const submit = () => {
    form.post(`/admin-pandawa/products/${props.product.id}`, {
        preserveScroll: true,
    });
};
</script>
...
<template>
  <div class="mb-8">
    ...
    <h2 class="text-3xl font-bold text-gray-900 tracking-tight">Edit Produk: {{ props.product.name }}</h2>
    <p class="text-gray-500 mt-2">Perbarui informasi detail untuk produk ini.</p>
  </div>
  ...
  <div class="bg-white rounded-3xl shadow-sm border border-emerald-50 overflow-hidden max-w-4xl">
    <form @submit.prevent="submit" class="p-8 space-y-6">
      ...
      <!-- Foto -->
      <div class="space-y-2">
        <label class="block text-sm font-bold text-gray-700">Foto Produk (Biarkan kosong jika tidak ingin mengubah)</label>
